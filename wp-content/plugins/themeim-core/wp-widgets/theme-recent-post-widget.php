...
class Themeim_Recent_Post_Widget extends WP_Widget

{
    public function __construct()
    {
        parent::__construct(
            'themeim_popular_posts',
            esc_html__('Themeim: Recent Posts with Thumbnails', 'themeim-core'),
            array('description' => esc_html__('Display your recent posts, with a thumbnail.', 'themeim-core'))
        );
    }
...

    {
        ...
        if ($recent_articles->have_posts()) : ?>
            ...
            esc_html__(' Oops, there are no posts.', 'themeim-core')
            ?>
        <?php endif; ?>
        </ul>
        <?php
...

    {
        ...
        if (!empty($instance) && $instance['title']) {
            ...
            $title = esc_html__('Recent Posts', 'themeim-core');
        ...
        $post_type = array(
            'post' => esc_html__('Blog Post Type', 'themeim-core'),
            'packages' => esc_html__('Packages Post Type', 'themeim-core'),
            'deals' => esc_html__('Deals Post Type', 'themeim-core'),
            'service' => esc_html__('Service Post Type', 'themeim-core'),
            'courses' => esc_html__('Courses Post Type', 'themeim-core')
        );
        ...
        $orderby_arr = array(
            ...
            'comment_count' => esc_html__('Most Comment', 'themeim-core')
        );

        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('post_type')) ?>"><?php esc_html_e('Post Type', 'themeim-core'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('no_of_posts')) ?>"><?php esc_html_e('No Of Posts', 'themeim-core'); ?></label>
...
if (!function_exists('Themeim_Recent_Post_Widget')) {

    function Themeim_Recent_Post_Widget()

    {
        register_widget('Themeim_Recent_Post_Widget');
    }

    add_action('widgets_init', 'Themeim_Recent_Post_Widget');
