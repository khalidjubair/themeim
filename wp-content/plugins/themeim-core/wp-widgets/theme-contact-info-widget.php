...
class Themeim_Contact_Info_Widget extends WP_Widget{

	public function __construct() {
		parent::__construct(
			'themeim_contact_info',
			esc_html__('Themeim: Contact Info','themeim-core'),
			array('description' => esc_html__('Display contact info widget','themeim-core'))
		);
	}
	...
	public function form($instance){
		...
		<?php foreach ( $contact_info as $ci ) :?>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id( $ci ) ); ?>"><?php echo esc_html( ucfirst( $ci ) . " " . esc_html__( 'Info', 'themeim-core' ) ); ?>
					: </label>
				<br/>
				...
				<small><?php echo esc_html__('Leave it blank if you don\'t want to show this info','themeim-core')?></small>
			</p>
...
if (!function_exists('Themeim_Contact_Info_Widget')){
	function Themeim_Contact_Info_Widget(){
		register_widget('Themeim_Contact_Info_Widget');
	}
	add_action('widgets_init','Themeim_Contact_Info_Widget');
}
