...
class flynext_Breadcrumb {
	/**
	 * Array of breadflynext_Breadcrumb
	 *
	 * @var array
	 */
	...
	public function __construct( array $options = [] ) {
		...
		$this->defaults['home'] = esc_html__('Home','flynext');
		$this->defaults['blog'] = esc_html__('Blog','flynext');
	...
	public function theBreadcrumbs() {
		...
		foreach ( $breadcrumbs as $crumb ) {
			...
			if ( $crumb['url'] ) {
                ...
                $html .= sprintf( '<span>%s</span>', wp_kses($crumb['title'],flynext()->kses_allowed_html('all')) );
			}
		...
		echo wp_kses($html,flynext()->kses_allowed_html('all'));
	}
	...
	public function getBreadcrumbs() {
		...
		$breadcrumbs = apply_filters( 'flynext_get_breadcrumbs', $breadcrumbs );
 ...
 * Template Functions.
 */
if ( ! function_exists( 'flynext_breadcrumb' ) ) {
	/**
	 ...
	 * @package flynext
	 */
	function flynext_breadcrumb( $options = [] ) {
		$Crumbs = new flynext_Breadcrumb( $options );
		$Crumbs->theBreadcrumbs();
	}
}

if ( ! function_exists( 'flynext_get_breadcrumbs' ) ) {
	/**
	 ...
	 * @return array
	 */
	function flynext_get_breadcrumbs( $options = [] ) {
		$Crumbs = new flynext_Breadcrumb( $options );
 ...
 * Utility functions
 */
if ( ! function_exists( 'flynext_breadcrumb_array_insert' ) ) {
	/**
	 ...
	 * @return array
	 */
	function flynext_breadcrumb_array_insert( array $array, $element, int $position ) {
