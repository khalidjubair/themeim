...
class themeim_Breadcrumb {
	/**
	 * Array of breadthemeim_Breadcrumb
	 *
	 * @var array
	 */
	...
	public function __construct( array $options = [] ) {
		...
		$this->defaults['home'] = esc_html__('Home','themeim');
		$this->defaults['blog'] = esc_html__('Blog','themeim');
	...
	public function theBreadcrumbs() {
		...
		foreach ( $breadcrumbs as $crumb ) {
			...
			if ( $crumb['url'] ) {
                ...
                $html .= sprintf( '<span>%s</span>', wp_kses($crumb['title'],themeim()->kses_allowed_html('all')) );
			}
		...
		echo wp_kses($html,themeim()->kses_allowed_html('all'));
	}
	...
	public function getBreadcrumbs() {
		...
		$breadcrumbs = apply_filters( 'themeim_get_breadcrumbs', $breadcrumbs );
 ...
 * Template Functions.
 */
if ( ! function_exists( 'themeim_breadcrumb' ) ) {
	/**
	 ...
	 * @package themeim
	 */
	function themeim_breadcrumb( $options = [] ) {
		$Crumbs = new themeim_Breadcrumb( $options );
		$Crumbs->theBreadcrumbs();
	}
}

if ( ! function_exists( 'themeim_get_breadcrumbs' ) ) {
	/**
	 ...
	 * @return array
	 */
	function themeim_get_breadcrumbs( $options = [] ) {
		$Crumbs = new themeim_Breadcrumb( $options );
 ...
 * Utility functions
 */
if ( ! function_exists( 'themeim_breadcrumb_array_insert' ) ) {
	/**
	 ...
	 * @return array
	 */
	function themeim_breadcrumb_array_insert( array $array, $element, int $position ) {
